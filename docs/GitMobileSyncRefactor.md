...
TidGi-Desktop owns the real repository and exposes two classes of functionality to the TiddlyWiki plugin:
...
1. Mobile fetches from the tracked desktop remote.
2. Mobile resets or fast-forwards as needed.
3. Mobile pushes local commits back to desktop.
4. If push conflicts, mobile uses a conflict branch strategy instead of interactive merge resolution.
...
- Git Smart HTTP through Desktop plus plugin.
- full-archive fast import path.
- mobile filesystem storage service.
- skinny HTML boot with streamed tiddlers.
- workspace routing to sub-workspaces.
- Git-based sync and conflict branch handling.
- mobile Git history and uncommitted change inspection.

Still evolving:

- large-repo uncommitted change performance, because statusMatrix remains expensive,
- consistency between WebView-visible content and repository-specific Git views,
- module registration filtering in the bundled skinny HTML runtime,
- importer UX for duplicate workspace IDs.
